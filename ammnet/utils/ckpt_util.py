...
def load_checkpoint(model, pretrained_path, prefix=None):
    ...
    if prefix is not None:
        ...
        for name, param in base_ckpt.items():
            new_ckpt[name.replace(prefix, '')] = param
        base_ckpt = new_ckpt
    new_ckpt = {}
    for name, para in base_ckpt.items():
        if 'complete.tsdf_net.' in name:
            new_name = name.replace('complete.tsdf_net.', 'complete.DownScale.')
        elif 'complete.tsdf_net2.' in name:
            new_name = name.replace('complete.tsdf_net2.', 'complete.UpScale.')
        else:
            new_name = name
        new_ckpt[new_name] = para
    base_ckpt = new_ckpt
    # state_dict['model'] = new_ckpt
    # torch.save(state_dict, pretrained_path.replace('.pth', '_newname.pth'))
